      ...
      <Card className="bg-card/80 border-white/10 shadow-lg"><CardContent className="p-0">
        ...
        {!isLoading && filtered.map((rfi) => { const overdue = isOverdue(rfi); return (
          <button key={rfi.id} onClick={() => setSelected(rfi)} className="group w-full grid grid-cols-12 gap-3 px-4 py-3 border-b last:border-b-0 border-white/10 text-left hover:bg-white/[0.03] transition-colors" data-testid={`row-rfi-${rfi.rfiNumber}`}>
            ...
            <div className="col-span-6 md:col-span-1 text-xs space-y-0.5">{rfi.costImpact && <span className="flex items-center gap-1 text-amber-400"><DollarSign className="h-3 w-3" />{rfi.costAmount ? fmtMoney(rfi.costAmount) : "TBD"}</span>}{rfi.scheduleImpactDays != null && rfi.scheduleImpactDays > 0 && <span className="flex items-center gap-1 text-orange-400"><Clock className="h-3 w-3" />+{rfi.scheduleImpactDays}d</span>}</div>
      ...
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        ...
        <div className="py-4 space-y-5">
          ...
          {!editing ? (<>
            ...
            <div className="grid grid-cols-2 gap-4"><Field icon={Layers} label="Spec section">{rfi.specSection ?? "—"}</Field><Field icon={FileText} label="Drawing ref">{rfi.drawingReference ?? "—"}</Field><Field icon={DollarSign} label="Cost impact">{rfi.costImpact ? (rfi.costAmount ? fmtMoney(rfi.costAmount) : "Yes — TBD") : "None"}</Field><Field icon={Clock} label="Schedule impact">{rfi.scheduleImpactDays != null && rfi.scheduleImpactDays > 0 ? `+${rfi.scheduleImpactDays} day(s)` : "None"}</Field><Field icon={AlertCircle} label="Priority"><Badge variant="outline" className={`capitalize ${PRIORITY_TONES[rfi.priority] ?? ""}`}>{rfi.priority}</Badge></Field><Field icon={Calendar} label="Due">{fmtDate(rfi.dueDate)}</Field></div>
          </>) : (<>
