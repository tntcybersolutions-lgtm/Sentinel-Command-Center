...
-   **Sentinel Hardening (Bugs 5-21)**: Production hardening pass across the GC platform. (a) Routing: `/bid` redirects to `/capture/pipeline`; `/systems-matrix`, `/building-systems`, `/estimate/systems` redirect to `/estimate/design-systems?tab=systems`; `/auto-build` redirects to `/estimate/design-systems?tab=overview`. (b) `getTabFromPath()` in `design-systems.tsx` reads `?tab=` query param and maps `/estimate/blueprints|takeoff|systems|deliverables` paths to the matching tab. (c) Quick Actions on Overview (`button-quick-takeoff`, `button-generate-deliverables`) use `setActiveTab(...)` instead of route navigation. (d) "Add System" and "Upload Plans" buttons converted from Radix `DialogTrigger asChild` to bare `Dialog` with explicit `onClick` (the Slot pattern silently breaks when nested inside other Radix components — same fix applied to all primary action buttons). (e) Materials sub-tab Cost column falls back to `qty * unitCost` when `extendedCost` is null. (f) Devices sub-tab now renders a real table from `/api/system-devices` filtered by `systemId` with empty-state messaging. (g) `POST /api/generate-deliverable/device_schedule` auto-seeds `system_devices` from per-`systemType` presets (structural/electrical/plumbing/hvac/drywall/doors/windows/lighting/lv_data/security/audio/smart_building/fire_life_safety) when none exist — preset keys MUST match canonical `SYSTEMS[].id` values from `design-systems.tsx`. (h) `POST /api/generate-deliverable/as_built` updates `building_systems.as_built_status = "submitted"` and invalidates the system cache. (i) `GET /api/project-deliverables/download/:type` is tenant-scoped, auto-generates the deliverable if missing, returns CSV (with `Content-Type: text/csv` + RFC-4180) for tabular types (`material_list`, `cable_schedule`, `device_schedule`, `trade_scope`) and `.txt` for document types. (j) `bid-readiness.tsx` Total Bids card uses `border-blue-200 text-blue-600` for visual hierarchy. (k) `GET /api/bids/readiness-dashboard` left-joins `opportunities` (since `bid_projects` has no `title`/`dueDate` columns — it uses `opportunities.title` + `opportunities.dueAt`) and reads `bid_readiness_scores.missingItemsJson`. (l) Sync Status mutation has `onError` toast and invalidates `/api/system-devices`. (m) Upload Drawing Sheets form blocks submit + shows "File required" toast when no file is selected, preserving the dialog state.
