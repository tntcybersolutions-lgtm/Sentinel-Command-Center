...
-   **Voice Daily Log Polish (Phase E)**: `/voice-daily-log` is a per-project field-capture page. Reads `selectedProjectId` from `useProjectContext()` with a `?projectId=` URL fallback that auto-fetches the project name via `/api/projects/:id` (avoids the "Project" sentinel from stale context). Shows three sections: (1) prominent "Record Voice Memo" button that opens a mock recorder dialog (real mic isn't wired) — tapping the simulated mic loads a sample transcript into the manual form; (2) manual entry form with the 5 spec fields (`weather`, `crew_count`, `tasks_completed`, `issues`, `safety_notes`) plus Clear/Save controls; (3) Recent Logs list with source badge (voice/manual), date, weather/crew/task/issue/safety counts, and `relTime` createdAt. New backend route `POST /api/projects/:id/daily-logs` registered BEFORE `registerSimpleProjectModuleRoutes` so it wins for the `dailyLogs` table (the generic loop targets a different `projectDailyLogs` table). Maps inputs → JSONB columns: `weather → {summary}`, `crew_count → laborJson [{count}]`, `tasks_completed/issues/safety_notes → string arrays` (newline OR array input). Empty state instructs picking a project from the sidebar Project Context selector.
